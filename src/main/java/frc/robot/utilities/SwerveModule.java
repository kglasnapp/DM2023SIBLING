...
public class SwerveModule {
  ...
  public static final double TURBO = 1.25;
  ...
  public static double getPowerRatio() {
    double inches = RobotContainer.instance.elevatorSubsystem.getElevatorLastPosInches();
    if (inches >= ELEVATOR_HEIGHT_LIMIT_FOR_SPEED) {
      return SwerveModule.PRECISION;
    }
    return powerRatio;
  }
  ...
  private void setSpeed(SwerveModuleState desiredState, boolean isOpenLoop) {
    if (isOpenLoop) {
      ...
      driveController.setReference(
          desiredState.speedMetersPerSecond * maxSpeed / getPowerRatio(),
          ControlType.kVelocity,
          0,
          feedforward.calculate(desiredState.speedMetersPerSecond * maxSpeed / getPowerRatio()));
    }
  }
  ...
  private void setAngle(SwerveModuleState desiredState) {
    ...
    //     currentAngleMod += 360;
    // }
    // double adjustedReferenceAngle = desiredAngle + currentAngle - currentAngleMod;
    ...
    if (Robot.count % 20 == 0) {
      ...
      // System.out.println("Module "+moduleNumber+" setting angle "+desiredState.angle.getDegrees());
    }

    angleController.setReference(angle, CANSparkMax.ControlType.kPosition);

    if (Robot.count % 10 == 0) {
      //System.out.println("Swerve Module "+moduleNumber+" trying to set "+angle.getDegrees()+" has an angle of "+integratedAngleEncoder.getPosition());
    }
